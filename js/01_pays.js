...
import { myAjaxPays } from "./00_init.js";
...
let retourParse = (reponse) => {
    ...
    tab.data = newObj.PAYS.records;
    tab.header = newObj.PAYS.columns;
...
function retourAjout(reponse) {
    ...
    myAjaxPays.getAll(retourParse, erreur);
};
...
let modifItem = (event) => {
    ...
    document.getElementById('okModif').addEventListener("click", function () {
        ...
        let nom = document.getElementById('idModif2').value;
        ajoutInfo = `NOMPAYS=${nom}`;
        myAjaxPays.Cle = code;
    },
        false);
};
...
let confirmModif = () => {
    myAjaxPays.put(ajoutInfo, retourModif, erreur);
};
...
let confirmSupr = () => {
    myAjaxPays.del(retourDel, erreur);
};
...
let ajoutItem = () => {
    ...
    document.getElementById('okAjout').addEventListener("click", function () {
        let nom = document.getElementById('idAjout2').value;
        ajoutInfo = `NOMPAYS=${nom}`;
    },
        false);
};
...
let confirmAjout = () => {
    myAjaxPays.post(ajoutInfo, retourAjout, erreur);
};

...
import { myAjaxCouleur } from "./00_init.js";
...
let retourParse = (reponse) => {
    ...
    tab.data = newObj.COULEUR.records;
    tab.header = newObj.COULEUR.columns;
...
function retourAjout(reponse) {
    ...
    myAjaxCouleur.getAll(retourParse, erreur);
};
...
let modifItem = (event) => {
    ...
    document.getElementById('okModif').addEventListener("click", function () {
        ...
        let couleur = document.getElementById('idModif2').value;
        ajoutInfo = `NOMCOULEUR=${couleur}`;
        myAjaxCouleur.Cle = code;
    },
        false);
};
...
let confirmModif = () => {
    myAjaxCouleur.put(ajoutInfo, retourModif, erreur);
};
...
let confirmSupr = () => {
    myAjaxCouleur.del(retourDel, erreur);
};
...
let ajoutItem = () => {
    ...
    document.getElementById('okAjout').addEventListener("click", function () {
        let couleur = document.getElementById('idAjout2').value;
        ajoutInfo = `NOMCOULEUR=${couleur}`;
    },
        false);
};
...
let confirmAjout = () => {
    myAjaxCouleur.post(ajoutInfo, retourAjout, erreur);
};
